...
export default async function Lobby( {params}: any ) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24 bg-cover bg-center" style={{ backgroundImage: "url('/murder-background.jpg')" }}>
      ...
      <div className="grid lg:grid-cols-5 w-full gap-10">
      {/**
       ...
       */}
        ...
        <div className="grid lg:col-span-3 justify-center gap-3">
          <div>
            { 
              !rows.some(row => row.email === session.user?.email) ? (
                <JoinButton gameid={game[0].gameid} email={session.user?.email ?? ""} />
              ) : (
                <button className="py-2.5 px-5 text-white bg-gray-300 rounded-full ">
                  You are in the game
                </button>
              )}

          { game[0].gamestate == 'open' ? (
            <a href={"/game/" + params.slug} className="py-2.5 px-5 text-white bg-pink-700 hover:bg-pink-600 rounded-full mx-auto">
                Start Game
            </a>
          ) : 
          (
